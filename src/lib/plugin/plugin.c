	...
	const struct bt_plugin_provider *plugin_provider)
{
	...
	for (uint64_t i = 0; i < plugin_count; i++) {
		...
		if (!plugin->info.provider) {
			plugin->info.provider = plugin_provider;
		} else {
			BT_ASSERT(plugin->info.provider == plugin_provider);
		}
	}
}
		...
		const struct bt_plugin_set **plugin_set_out)
{
	...
	const struct bt_plugin_provider_set *provider_set = NULL;
	...
	status = try_init_plugin_providers(&provider_set);
	if (status < 0) {
	...
	if (provider_set) {
		...
		for (size_t i = 0; i < provider_set->plugin_providers->len; i++) {
			struct bt_plugin_provider *plugin_provider =
				g_ptr_array_index(provider_set->plugin_providers, i);
			...
			if (status == BT_FUNC_STATUS_OK) {
				BT_ASSERT_POST(
					...
					"%![plugin-provider-]+U",
					plugin_provider);
				set_provider_of_plugins(scratch_plugin_set,
					plugin_provider);
...
bt_plugin_borrow_provider(const struct bt_plugin *plugin)
{
	BT_ASSERT_PRE_DEV_PLUGIN_NON_NULL(plugin);
	return plugin->info.provider;
}
