...
#define BT_AUTODISC_LOG_AND_APPEND(_lvl, _fmt, ...)                                                \
    do {                                                                                           \
        ...
        (void) BT_CURRENT_THREAD_ERROR_APPEND_CAUSE_FROM_UNKNOWN("Source auto-discovery", _fmt,    \
                                                                 ##__VA_ARGS__);                   \
    } while (0)
...
static void auto_source_discovery_result_destroy(struct auto_source_discovery_result *res)
{
    if (res) {
        g_free(res->group);
...
auto_source_discovery_result_create(const char *plugin_name, const char *source_cc_name,
...
int auto_source_discovery_init(struct auto_source_discovery *auto_disc)
{
    ...
    auto_disc->results =
        g_ptr_array_new_with_free_func((GDestroyNotify) auto_source_discovery_result_destroy);
...
static const bt_value *borrow_array_value_last_element_const(const bt_value *array)
{
...
static auto_source_discovery_internal_status
auto_source_discovery_add(struct auto_source_discovery *auto_disc, const char *plugin_name,
                          const char *source_cc_name, const char *group, const char *input,
                          uint64_t original_input_index, bt_logging_level log_level)
{
    ...
    if (group) {
        for (i = 0; i < len; i++) {
            res = (auto_source_discovery_result *) g_ptr_array_index(auto_disc->results, i);
    ...
    if (i == len) {
        ...
        res = auto_source_discovery_result_create(plugin_name, source_cc_name, group, log_level);
        if (!res) {
            goto error;
        }
    ...
    if (bt_value_array_is_empty(res->original_input_indices)) {
        ...
        last_index_value = borrow_array_value_last_element_const(res->original_input_indices);
    ...
    if (append_index) {
        append_status = bt_value_array_append_unsigned_integer_element(res->original_input_indices,
...
static int convert_weight_value(const bt_value *weight_value, double *weight,
...
static bt_query_executor_query_status simple_query(const bt_component_class *comp_cls,
                                                   ...
                                                   const bt_value **result)
{
    ...
    if (set_logging_level_status != BT_QUERY_EXECUTOR_SET_LOGGING_LEVEL_STATUS_OK) {
        BT_AUTODISC_LOGE_APPEND_CAUSE("Cannot set query executor's logging level: "
                                      "log-level=%s",
 ...
 * with that name.
 */
static auto_source_discovery_internal_status support_info_query_all_sources(
    const char *input, const char *input_type, uint64_t original_input_index,
    const bt_plugin **plugins, size_t plugin_count, const char *component_class_restrict,
    enum bt_logging_level log_level, struct auto_source_discovery *auto_disc,
    const bt_interrupter *interrupter)
{
    ...
    for (i_plugins = 0; i_plugins < plugin_count; i_plugins++) {
        ...
        for (i_sources = 0; i_sources < source_count; i_sources++) {
            ...
            BT_LOGD(
                "babeltrace.support-info query: before: component-class-name=source.%s.%s, input=%s, "
                "type=%s",
                plugin_name, source_cc_name, input, input_type);

            BT_VALUE_PUT_REF_AND_RESET(query_result);
            query_status =
                simple_query(cc, "babeltrace.support-info", query_params, log_level, &query_result);
            ...
            if (query_status == BT_QUERY_EXECUTOR_QUERY_STATUS_OK) {
                ...
                if (query_result_type == BT_VALUE_TYPE_REAL ||
                    query_result_type == BT_VALUE_TYPE_SIGNED_INTEGER) {
                    if (convert_weight_value(query_result, &weight, plugin_name, source_cc_name,
                                             input, input_type, log_level) != 0) {
                    ...
                    if (convert_weight_value(weight_value, &weight, plugin_name, source_cc_name,
                                             input, input_type, log_level) != 0) {
                        /* convert_weight_value has already warned. */
                        continue;
                    }
                    ...
                    if (bt_value_map_has_entry(query_result, "group")) {
                        ...
                        if (bt_value_get_type(group_value) != BT_VALUE_TYPE_STRING) {
                            BT_LOGW(
                                "babeltrace.support-info query: unexpected type for entry `group`: "
                                "component-class-name=source.%s.%s, input=%s, input-type=%s, "
                                "expected-entry-type=%s,%s, actual-entry-type=%s",
                                bt_plugin_get_name(plugin), bt_component_class_get_name(cc), input,
                                input_type, bt_common_value_type_string(BT_VALUE_TYPE_NULL),
                    ...
                    BT_LOGW("babeltrace.support-info query: unexpected result type: "
                            ...
                            bt_plugin_get_name(plugin), bt_component_class_get_name(cc), input,
                            input_type, bt_common_value_type_string(BT_VALUE_TYPE_REAL),
                ...
                BT_LOGD(
                    "babeltrace.support-info query: success: component-class-name=source.%s.%s, input=%s, "
                    "type=%s, weight=%f, group=%s",
                    bt_plugin_get_name(plugin), bt_component_class_get_name(cc), input, input_type,
                    weight, group_value ? bt_value_string_get(group_value) : "(none)");
                ...
                BT_LOGD(
                    "babeltrace.support-info query: failure: component-class-name=source.%s.%s, input=%s, "
                    "type=%s, status=%s",
                    bt_plugin_get_name(plugin), bt_component_class_get_name(cc), input, input_type,
    ...
    if (winner.source) {
        ...
        BT_LOGI(
            "Input awarded: input=%s, type=%s, component-class-name=source.%s.%s, weight=%f, group=%s",
        ...
        status = auto_source_discovery_add(auto_disc, plugin_name, source_name, group, input,
                                           original_input_index, log_level);
        if (status != AUTO_SOURCE_DISCOVERY_INTERNAL_STATUS_OK) {
            goto error;
        }
    } else {
        BT_LOGI("Input not recognized: input=%s, type=%s", input, input_type);
...
static auto_source_discovery_internal_status auto_discover_source_for_input_as_string(
    const char *input, uint64_t original_input_index, const bt_plugin **plugins,
    size_t plugin_count, const char *component_class_restrict, enum bt_logging_level log_level,
    struct auto_source_discovery *auto_disc, const bt_interrupter *interrupter)
{
    return support_info_query_all_sources(input, "string", original_input_index, plugins,
                                          plugin_count, component_class_restrict, log_level,
                                          auto_disc, interrupter);
}

static auto_source_discovery_internal_status auto_discover_source_for_input_as_dir_or_file_rec(
    GString *input, uint64_t original_input_index, const bt_plugin **plugins, size_t plugin_count,
    const char *component_class_restrict, enum bt_logging_level log_level,
    struct auto_source_discovery *auto_disc, const bt_interrupter *interrupter)
{
    auto_source_discovery_internal_status status;
    GError *error = NULL;
    ...
    if (g_file_test(input->str, G_FILE_TEST_IS_REGULAR)) {
        /* It's a file. */
        status = support_info_query_all_sources(input->str, "file", original_input_index, plugins,
                                                plugin_count, component_class_restrict, log_level,
                                                auto_disc, interrupter);
    } else if (g_file_test(input->str, G_FILE_TEST_IS_DIR)) {
        const gchar *dirent;
        gsize saved_input_len;
        auto_source_discovery_internal_status dir_status =
            AUTO_SOURCE_DISCOVERY_INTERNAL_STATUS_NO_MATCH;

        /* It's a directory. */
        status = support_info_query_all_sources(input->str, "directory", original_input_index,
...
#define BT_FMT "Failed to open directory %s: %s"
            ...
            if (error->code == G_FILE_ERROR_ACCES) {
                ...
                BT_AUTODISC_LOGE_APPEND_CAUSE(BT_FMT, input->str, error->message);
                goto error;
            }
#undef BT_FMT
        }
        ...
        do {
            ...
            if (dirent) {
                ...
                status = auto_discover_source_for_input_as_dir_or_file_rec(
                    input, original_input_index, plugins, plugin_count, component_class_restrict,
                    log_level, auto_disc, interrupter);
...
static auto_source_discovery_internal_status auto_discover_source_for_input_as_dir_or_file(
    const char *input, uint64_t original_input_index, const bt_plugin **plugins,
    size_t plugin_count, const char *component_class_restrict, enum bt_logging_level log_level,
    struct auto_source_discovery *auto_disc, const bt_interrupter *interrupter)
{
    ...
    status = auto_discover_source_for_input_as_dir_or_file_rec(
        mutable_input, original_input_index, plugins, plugin_count, component_class_restrict,
        log_level, auto_disc, interrupter);
...
auto_source_discovery_status auto_discover_source_components(
    const bt_value *inputs, const bt_value *log_levels, const bt_plugin **plugins,
    size_t plugin_count, const char *component_class_restrict,
    enum bt_logging_level default_log_level, struct auto_source_discovery *auto_disc,
    const bt_interrupter *interrupter)
{
    ...
    for (i_inputs = 0; i_inputs < input_count; i_inputs++) {
        ...
        if (bt_value_get_type(log_level_value) == BT_VALUE_TYPE_UNSIGNED_INTEGER) {
             ...
             */
            log_level =
                static_cast<bt_logging_level>(bt_value_integer_unsigned_get(log_level_value));
        } else {
            BT_ASSERT(bt_value_get_type(log_level_value) == BT_VALUE_TYPE_NULL);
            log_level = default_log_level;
        }

        internal_status = auto_discover_source_for_input_as_string(
            input, i_inputs, plugins, plugin_count, component_class_restrict, log_level, auto_disc,
            interrupter);
        if (internal_status < 0 ||
            internal_status == AUTO_SOURCE_DISCOVERY_INTERNAL_STATUS_INTERRUPTED) {
        ...
        internal_status = auto_discover_source_for_input_as_dir_or_file(
            input, i_inputs, plugins, plugin_count, component_class_restrict, log_level, auto_disc,
            interrupter);
        if (internal_status < 0 ||
            internal_status == AUTO_SOURCE_DISCOVERY_INTERNAL_STATUS_INTERRUPTED) {
