...
/* sink.text.pretty */
BT_CPP_PLUGIN_SINK_COMPONENT_CLASS(pretty, bt2pretty::Comp);

/* dmesg source */
BT_PLUGIN_SOURCE_COMPONENT_CLASS(dmesg, dmesg_msg_iter_next);
BT_PLUGIN_SOURCE_COMPONENT_CLASS_DESCRIPTION(
    dmesg, "Read Linux ring buffer lines (dmesg(1) output) from a file or from standard input.");
...
BT_PLUGIN_SOURCE_COMPONENT_CLASS_MESSAGE_ITERATOR_CLASS_SEEK_BEGINNING_METHODS(
    dmesg, dmesg_msg_iter_seek_beginning, dmesg_msg_iter_can_seek_beginning);
...
BT_PLUGIN_SINK_COMPONENT_CLASS_GRAPH_IS_CONFIGURED_METHOD(details, details_graph_is_configured);
