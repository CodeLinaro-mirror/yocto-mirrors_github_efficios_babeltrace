...
static struct bt_config_component *find_component_in_array(GPtrArray *comps, const char *name)
{
    size_t i;
...
static struct bt_config_component *find_component(struct bt_config *cfg, const char *name)
{
    struct bt_config_component *comp;
                                        ...
                                        size_t error_buf_size)
{
    ...
    for (i = 0; i < cfg->cmd_data.run.connections->len; i++) {
        struct bt_config_connection *connection = static_cast<bt_config_connection *>(
        ...
        if (!comp) {
            snprintf(error_buf, error_buf_size,
                     ...
                     connection->upstream_comp_name->str, connection->arg->str);
            ret = -1;
            goto end;
        }
        ...
        if (!comp) {
            snprintf(error_buf, error_buf_size,
                     ...
                     connection->downstream_comp_name->str, connection->arg->str);
            ret = -1;
            goto end;
        }
    }
...
static int validate_connection_directions(struct bt_config *cfg, char *error_buf,
                                          size_t error_buf_size)
{
    size_t i;
    int ret = 0;
    ...
    for (i = 0; i < cfg->cmd_data.run.connections->len; i++) {
        ...
        src_comp = find_component(cfg, connection->upstream_comp_name->str);
        BT_ASSERT(src_comp);
        dst_comp = find_component(cfg, connection->downstream_comp_name->str);
        BT_ASSERT(dst_comp);
        ...
        if (src_comp->type == BT_COMPONENT_CLASS_TYPE_SOURCE) {
            ...
            snprintf(
                ...
                connection->upstream_comp_name->str, connection->downstream_comp_name->str,
                connection->arg->str);
            ret = -1;
            goto end;
        }
...
static int validate_no_cycles_rec(struct bt_config *cfg, GPtrArray *path, char *error_buf,
                                  size_t error_buf_size)
{
    int ret = 0;
    size_t conn_i;
    ...
    src_comp_name = static_cast<const char *>(g_ptr_array_index(path, path->len - 1));

    for (conn_i = 0; conn_i < cfg->cmd_data.run.connections->len; conn_i++) {
        struct bt_config_connection *conn = static_cast<bt_config_connection *>(
        ...
        if (strcmp(conn->upstream_comp_name->str, src_comp_name) == 0) {
            ...
            for (path_i = 0; path_i < path->len; path_i++) {
                const char *comp_name = static_cast<const char *>(g_ptr_array_index(path, path_i));
            ...
            ret = validate_no_cycles_rec(cfg, path, error_buf, error_buf_size);
            if (ret) {
                goto end;
            }
...
static int validate_no_cycles(struct bt_config *cfg, char *error_buf, size_t error_buf_size)
{
    size_t i;
    int ret = 0;
    GPtrArray *path;
                                                      ...
                                                      char *error_buf, size_t error_buf_size)
{
    ...
    for (i = 0; i < comps->len; i++) {
        ...
        if (!bt_value_map_has_entry(connected_components, comp->instance_name->str)) {
            snprintf(error_buf, error_buf_size, "Component `%s` is not connected\n",
...
static int validate_all_components_connected(struct bt_config *cfg, char *error_buf,
                                             size_t error_buf_size)
{
    size_t i;
    int ret = 0;
    ...
    for (i = 0; i < cfg->cmd_data.run.connections->len; i++) {
        ...
        ret = bt_value_map_insert_entry(connected_components, connection->upstream_comp_name->str,
                                        bt_value_null);
        if (ret) {
            goto end;
        }

        ret = bt_value_map_insert_entry(connected_components, connection->downstream_comp_name->str,
                                        bt_value_null);
        if (ret) {
            goto end;
        }
    ...
    ret = validate_all_components_connected_in_array(
        cfg->cmd_data.run.sources, connected_components, error_buf, error_buf_size);
    if (ret) {
        goto end;
    }

    ret = validate_all_components_connected_in_array(
        cfg->cmd_data.run.filters, connected_components, error_buf, error_buf_size);
    if (ret) {
        goto end;
    }

    ret = validate_all_components_connected_in_array(cfg->cmd_data.run.sinks, connected_components,
                                                     error_buf, error_buf_size);
    if (ret) {
        goto end;
    }
...
static int validate_no_duplicate_connection(struct bt_config *cfg, char *error_buf,
                                            size_t error_buf_size)
{
    size_t i;
    int ret = 0;
    bt_value *flat_connection_names = bt_value_map_create();
    ...
    for (i = 0; i < cfg->cmd_data.run.connections->len; i++) {
        ...
        g_string_printf(flat_connection_name, "%s\x01%s\x01%s\x01%s",
                        connection->upstream_comp_name->str, connection->upstream_port_glob->str,
                        connection->downstream_comp_name->str,
                        connection->downstream_port_glob->str);

        if (bt_value_map_has_entry(flat_connection_names, flat_connection_name->str)) {
            snprintf(error_buf, error_buf_size, "Duplicate connection:\n    %s\n",
                     connection->arg->str);
            ret = -1;
            goto end;
        }

        ret = bt_value_map_insert_entry(flat_connection_names, flat_connection_name->str,
                                        bt_value_null);
        if (ret) {
            goto end;
        }
...
static int validate_connections(struct bt_config *cfg, char *error_buf, size_t error_buf_size)
{
    int ret;
                                          ...
                                          char *error_buf, size_t error_buf_size)
{
    ...
    for (i = 0; i < bt_value_array_get_length(connection_args); i++) {
        const bt_value *arg_value =
            bt_value_array_borrow_element_by_index_const(connection_args, i);
        const char *arg;
        struct bt_config_connection *cfg_connection;
        ...
        g_ptr_array_add(cfg->cmd_data.run.connections, cfg_connection);
    }
