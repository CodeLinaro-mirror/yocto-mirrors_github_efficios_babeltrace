...
static void set_fail_reason(const char **fail_reason, const char *reason)
{
    if (fail_reason) {
        *fail_reason = reason;
    }
                                         ...
                                         const bt_value **user_result, const char **fail_reason)
{
    ...
    if (bt_query_executor_set_logging_level(query_exec, log_level) !=
        ...
        BT_CLI_LOGE_APPEND_CAUSE("Cannot set query executor's logging level: "
                                 "log-level=%s",
    ...
    if (interrupter) {
        if (bt_query_executor_add_interrupter(query_exec, interrupter) !=
            BT_QUERY_EXECUTOR_ADD_INTERRUPTER_STATUS_OK) {
            BT_CLI_LOGE_APPEND_CAUSE("Cannot add interrupter to query executor.");
            goto error;
        }
    }
    ...
    while (true) {
        ...
        case BT_QUERY_EXECUTOR_QUERY_STATUS_AGAIN:
        {
            ...
            if (usleep(sleep_time_us)) {
                if (interrupter && bt_interrupter_is_set(interrupter)) {
                    BT_CLI_LOGW_APPEND_CAUSE("Query was interrupted by user: "
                                             "comp-cls-addr=%p, comp-cls-name=\"%s\", "
                                             "query-obj=\"%s\"",
                                             comp_cls, bt_component_class_get_name(comp_cls), obj);
                    set_fail_reason(fail_reason, "interrupted by user");
                    goto error;
                }
            }
        ...
        case BT_QUERY_EXECUTOR_QUERY_STATUS_MEMORY_ERROR:
            ...
            goto error;
        default:
            BT_LOGF("Unknown query status: status=%s", bt_common_func_status_string(status));
            bt_common_abort();
        }
    }
