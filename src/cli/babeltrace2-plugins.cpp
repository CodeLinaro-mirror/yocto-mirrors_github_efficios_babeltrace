...
{
    loaded_plugins = g_ptr_array_new_with_free_func((GDestroyNotify) bt_plugin_put_ref);
}
...
const bt_plugin *borrow_loaded_plugin_by_name(const char *name)
{
    ...
    for (i = 0; i < loaded_plugins->len; i++) {
        plugin = static_cast<const bt_plugin *>(g_ptr_array_index(loaded_plugins, i));
    ...
    if (plugin) {
        BT_LOGI("Found plugin: name=\"%s\", plugin-addr=%p", name, plugin);
    } else {
...
const bt_plugin *borrow_loaded_plugin_by_index(size_t index)
{
    ...
    return static_cast<const bt_plugin *>(g_ptr_array_index(loaded_plugins, index));
}

static void add_to_loaded_plugins(const bt_plugin_set *plugin_set)
{
    int64_t i;
    int64_t count;
    ...
    for (i = 0; i < count; i++) {
        ...
        const bt_plugin *loaded_plugin = borrow_loaded_plugin_by_name(bt_plugin_get_name(plugin));
        ...
        if (loaded_plugin) {
            BT_LOGI("Not using plugin: another one already exists with the same name: "
                    ...
                    bt_plugin_get_name(plugin), bt_plugin_get_path(plugin),
...
static int load_dynamic_plugins(const bt_value *plugin_paths)
{
    ...
    for (i = 0; i < nr_paths; i++) {
        ...
        plugin_path_value = bt_value_array_borrow_element_by_index_const(plugin_paths, i);
        ...
        status = bt_plugin_find_all_from_dir(plugin_path, BT_FALSE, BT_TRUE, &plugin_set);
        if (status < 0) {
            BT_CLI_LOGE_APPEND_CAUSE("Unable to load dynamic plugins from directory: "
                                     "path=\"%s\"",
                                     plugin_path);
            ret = status;
            goto end;
        } else if (status == BT_PLUGIN_FIND_ALL_FROM_DIR_STATUS_NOT_FOUND) {
            BT_LOGI("No plugins found in directory: path=\"%s\"", plugin_path);
            continue;
        }
...
static int load_static_plugins(void)
{
    ...
    if (status < 0) {
        ...
        ret = -1;
        goto end;
    } else if (status == BT_PLUGIN_FIND_ALL_FROM_STATIC_STATUS_NOT_FOUND) {
...
int require_loaded_plugins(const bt_value *plugin_paths)
{
    ...
    /*
     * Load static (built-in) plugins first so that they take precedence
     * over dynamic plugins.
     */
