 ...
 */
static void plugin_comp_cls_names(const char *arg, char **name, char **plugin, char **comp_cls,
                                  bt_component_class_type *comp_cls_type)
{
    ...
    if (strcmp(gs_comp_cls_type->str, "source") == 0 || strcmp(gs_comp_cls_type->str, "src") == 0) {
        ...
        *comp_cls_type = BT_COMPONENT_CLASS_TYPE_SINK;
        ...
        BT_CLI_LOGE_APPEND_CAUSE("Unknown component class type: `%s`.", gs_comp_cls_type->str);
        goto error;
    }
...
static void print_and_indent(const char *str)
{
    const char *ch = &str[0];
...
static void print_version(void)
{
    ...
    bool has_extra = has_extra_name || has_extra_description || has_extra_patch_names;

    printf("%sBabeltrace %s%s", bt_common_color_bold(), VERSION, bt_common_color_reset());

    if (strlen(BT_VERSION_NAME) > 0) {
        printf(" \"%s%s%s%s\"", bt_common_color_fg_bright_blue(), bt_common_color_bold(),
               BT_VERSION_NAME, bt_common_color_reset());
    }

    if (strlen(BT_VERSION_GIT) > 0) {
        printf(" [%s%s%s]", bt_common_color_fg_yellow(), BT_VERSION_GIT, bt_common_color_reset());
    }
 ...
 */
static void bt_config_component_destroy(bt_object *obj)
{
 ...
 */
static struct bt_config_component *bt_config_component_create(bt_component_class_type type,
                                                              ...
                                                              int init_log_level)
{
    ...
    bt_object_init_shared(&cfg_component->base, bt_config_component_destroy);
 ...
 * option's argument.
 */
static struct bt_config_component *bt_config_component_from_arg(const char *arg, int init_log_level)
{
    ...
    cfg_comp = bt_config_component_create(type, plugin_name, comp_cls_name, init_log_level);
    if (!cfg_comp) {
        goto error;
    }
 ...
 */
static void bt_config_destroy(bt_object *obj)
{
    struct bt_config *cfg = container_of(obj, struct bt_config, base);
    ...
    case BT_CONFIG_COMMAND_RUN:
        ...
        if (cfg->cmd_data.run.connections) {
            g_ptr_array_free(cfg->cmd_data.run.connections, TRUE);
        }
        break;
    ...
    case BT_CONFIG_COMMAND_PRINT_CTF_METADATA:
        if (cfg->cmd_data.print_ctf_metadata.path) {
            g_string_free(cfg->cmd_data.print_ctf_metadata.path, TRUE);
            g_string_free(cfg->cmd_data.print_ctf_metadata.output_path, TRUE);
        }
        break;
    case BT_CONFIG_COMMAND_PRINT_LTTNG_LIVE_SESSIONS:
        if (cfg->cmd_data.print_lttng_live_sessions.url) {
            g_string_free(cfg->cmd_data.print_lttng_live_sessions.url, TRUE);
            g_string_free(cfg->cmd_data.print_lttng_live_sessions.output_path, TRUE);
        }
        break;
    default:
        bt_common_abort();
 ...
 * Return value is owned by the caller.
 */
static GScanner *create_csv_identifiers_scanner(void)
{
    GScanner *scanner;
 ...
 * Return value is owned by the caller.
 */
static bt_value *names_from_arg(const char *arg)
{
    ...
    while (true) {
        ...
        case G_TOKEN_IDENTIFIER:
        {
            ...
            if (strcmp(identifier, "payload") == 0 || strcmp(identifier, "args") == 0 ||
                strcmp(identifier, "arg") == 0) {
                found_item = true;
                if (bt_value_array_append_string_element(names, "payload")) {
                    goto error;
                }
            } else if (strcmp(identifier, "context") == 0 || strcmp(identifier, "ctx") == 0) {
                found_item = true;
                if (bt_value_array_append_string_element(names, "context")) {
                    goto error;
                }
            } else if (strcmp(identifier, "scope") == 0 || strcmp(identifier, "header") == 0) {
                found_item = true;
                ...
                BT_CLI_LOGE_APPEND_CAUSE("Unknown name: `%s`.", identifier);
                goto error;
            }
            break;
        }
    ...
    if (found_none && found_all) {
        BT_CLI_LOGE_APPEND_CAUSE(
            "Only either `all` or `none` can be specified in the list given to the --names option, but not both.");
        goto error;
    }
    /*
 ...
 * Return value is owned by the caller.
 */
static bt_value *fields_from_arg(const char *arg)
{
    GScanner *scanner = NULL;
    bt_value *fields;
    ...
    while (true) {
        ...
        case G_TOKEN_IDENTIFIER:
        {
            ...
            if (strcmp(identifier, "trace") == 0 || strcmp(identifier, "trace:hostname") == 0 ||
                strcmp(identifier, "trace:domain") == 0 ||
                strcmp(identifier, "trace:procname") == 0 ||
                strcmp(identifier, "trace:vpid") == 0 || strcmp(identifier, "loglevel") == 0 ||
                strcmp(identifier, "emf") == 0 || strcmp(identifier, "callsite") == 0 ||
                strcmp(identifier, "all") == 0) {
                if (bt_value_array_append_string_element(fields, identifier)) {
                    goto error;
                }
            } else {
                BT_CLI_LOGE_APPEND_CAUSE("Unknown field: `%s`.", identifier);
                goto error;
            }
            break;
        }
...
static void append_param_arg(GString *params_arg, const char *key, const char *value)
{
 ...
 */
static int insert_flat_params_from_array(GString *params_arg, const bt_value *names_array,
                                         const char *prefix)
{
    int ret = 0;
    uint64_t i;
    ...
    for (i = 0; i < bt_value_array_get_length(names_array); i++) {
        const bt_value *str_obj = bt_value_array_borrow_element_by_index_const(names_array, i);
        const char *suffix;
 ...
 */
static void add_run_cfg_comp(struct bt_config *cfg, struct bt_config_component *cfg_comp,
...
static int add_run_cfg_comp_check_name(struct bt_config *cfg, struct bt_config_component *cfg_comp,
                                       enum bt_config_component_dest dest, bt_value *instance_names)
{
    int ret = 0;
    ...
    if (bt_value_map_insert_entry(instance_names, cfg_comp->instance_name->str, bt_value_null)) {
        BT_CLI_LOGE_APPEND_CAUSE_OOM();
        ret = -1;
        goto end;
    }
...
static int append_env_var_plugin_paths(bt_value *plugin_paths)
{
    int ret = 0;
    const char *envvar;
...
static int append_home_and_system_plugin_paths(bt_value *plugin_paths, bool omit_system_plugin_path,
                                               bool omit_home_plugin_path)
{
    int ret;
    const char *disable_std_dirs_envvar = getenv("BABELTRACE_DISABLE_STD_PLUGIN_DIRS");
    const bool disable_std_dirs =
        disable_std_dirs_envvar && strcmp(disable_std_dirs_envvar, "1") == 0;
    ...
    if (!omit_home_plugin_path && !disable_std_dirs) {
        if (bt_common_is_setuid_setgid()) {
            ...
            char *home_plugin_dir = bt_common_get_home_plugin_path(BT_LOG_OUTPUT_LEVEL);

            if (home_plugin_dir) {
                ret = bt_config_append_plugin_paths(plugin_paths, home_plugin_dir);
    ...
    if (!omit_system_plugin_path && !disable_std_dirs) {
        if (bt_config_append_plugin_paths(plugin_paths, bt_common_get_system_plugin_path())) {
                                               ...
                                               const bt_value *plugin_paths, bool needs_plugins)
{
    ...
    if (plugin_paths) {
        ...
        (void) bt_value_copy(plugin_paths, &plugin_paths_copy);
...
static struct bt_config *bt_config_run_create(const bt_value *plugin_paths)
{
    struct bt_config *cfg;

    /* Create config */
    cfg = bt_config_base_create(BT_CONFIG_COMMAND_RUN, plugin_paths, true);
    if (!cfg) {
        goto error;
    }

    cfg->cmd_data.run.sources = g_ptr_array_new_with_free_func((GDestroyNotify) bt_object_put_ref);
    if (!cfg->cmd_data.run.sources) {
        BT_CLI_LOGE_APPEND_CAUSE_OOM();
        goto error;
    }

    cfg->cmd_data.run.filters = g_ptr_array_new_with_free_func((GDestroyNotify) bt_object_put_ref);
    if (!cfg->cmd_data.run.filters) {
        BT_CLI_LOGE_APPEND_CAUSE_OOM();
        goto error;
    }

    cfg->cmd_data.run.sinks = g_ptr_array_new_with_free_func((GDestroyNotify) bt_object_put_ref);
    if (!cfg->cmd_data.run.sinks) {
        BT_CLI_LOGE_APPEND_CAUSE_OOM();
        goto error;
    }

    cfg->cmd_data.run.connections =
        g_ptr_array_new_with_free_func((GDestroyNotify) bt_config_connection_destroy);
    if (!cfg->cmd_data.run.connections) {
        BT_CLI_LOGE_APPEND_CAUSE_OOM();
        goto error;
    }
...
static struct bt_config *bt_config_list_plugins_create(const bt_value *plugin_paths)
{
    return bt_config_base_create(BT_CONFIG_COMMAND_LIST_PLUGINS, plugin_paths, true);
}

static struct bt_config *bt_config_list_plugin_providers_create(void)
{
    return bt_config_base_create(BT_CONFIG_COMMAND_LIST_PLUGIN_PROVIDERS, NULL, false);
}

static struct bt_config *bt_config_help_create(const bt_value *plugin_paths, int default_log_level)
{
    struct bt_config *cfg;

    /* Create config */
    cfg = bt_config_base_create(BT_CONFIG_COMMAND_HELP, plugin_paths, true);
    if (!cfg) {
        goto error;
    }

    cfg->cmd_data.help.cfg_component = bt_config_component_create(
        static_cast<bt_component_class_type>(-1), NULL, NULL, default_log_level);
...
static struct bt_config *bt_config_query_create(const bt_value *plugin_paths)
{
    struct bt_config *cfg;

    /* Create config */
    cfg = bt_config_base_create(BT_CONFIG_COMMAND_QUERY, plugin_paths, true);
    if (!cfg) {
        goto error;
    }
...
static enum bt_config_cli_args_status
bt_config_print_ctf_metadata_create(const bt_value *plugin_paths, struct bt_config **cfg_out)
{
    struct bt_config *cfg;
    enum bt_config_cli_args_status status;

    /* Create config */
    cfg = bt_config_base_create(BT_CONFIG_COMMAND_PRINT_CTF_METADATA, plugin_paths, true);
    if (!cfg) {
        goto error;
    }
...
static enum bt_config_cli_args_status
bt_config_print_lttng_live_sessions_create(const bt_value *plugin_paths, struct bt_config **cfg_out)
{
    struct bt_config *cfg;
    enum bt_config_cli_args_status status;

    /* Create config */
    cfg = bt_config_base_create(BT_CONFIG_COMMAND_PRINT_LTTNG_LIVE_SESSIONS, plugin_paths, true);
    if (!cfg) {
        goto error;
    }
    ...
    cfg->cmd_data.print_lttng_live_sessions.output_path = g_string_new(NULL);
...
static int bt_config_append_plugin_paths_check_setuid_setgid(bt_value *plugin_paths,
                                                             const char *arg)
{
    int ret = 0;
 ...
 */
static void print_expected_params_format(FILE *fp)
{
 ...
 */
static GString *__BT_ATTR_FORMAT_PRINTF(4, 5)
    format_arg_error(const struct argpar_error *error, const char **argv,
                     unsigned int arg_index_offset, const char *prefix_fmt, ...)
{
    ...
    case ARGPAR_ERROR_TYPE_MISSING_OPT_ARG:
    {
        ...
        if (is_short) {
            g_string_append_printf(
                str, WHILE_PARSING_ARG_N_FMT "Missing required argument for option `-%c`",
                arg_index_offset + orig_index + 1, arg, descr->short_name);
        } else {
            g_string_append_printf(
                str, WHILE_PARSING_ARG_N_FMT "Missing required argument for option `--%s`",
    ...
    case ARGPAR_ERROR_TYPE_UNKNOWN_OPT:
    {
        ...
        g_string_append_printf(str, WHILE_PARSING_ARG_N_FMT "Unknown option `%s`",
 ...
 * returned in `*item`.
 */
static enum parse_next_item_status parse_next_item(struct argpar_iter *iter,
                                                   const struct argpar_item **item,
                                                   const char **argv, const char *command,
                                                   unsigned int consumed_args)
{
    ...
    {
        GString *err_str =
            format_arg_error(error, argv, consumed_args,
 ...
 */
static void print_help_usage(FILE *fp)
{
    /* clang-format off */
...
static const struct argpar_opt_descr help_options[] = {
 ...
 * command.
 */
static enum bt_config_cli_args_status bt_config_help_from_args(int argc, const char *argv[],
                                                               struct bt_config **cfg_out,
                                                               ...
                                                               unsigned int consumed_args)
{
    ...
    while (true) {
        enum parse_next_item_status parse_status =
            parse_next_item(argpar_iter, &argpar_item, argv, "help", consumed_args);
        ...
        if (argpar_item_type(argpar_item) == ARGPAR_ITEM_TYPE_OPT) {
            const struct argpar_opt_descr *opt_descr = argpar_item_opt_descr(argpar_item);
            ...
            if (plugin_comp_cls_arg) {
                BT_CLI_LOGE_APPEND_CAUSE(
                    "Extraneous command-line argument specified to `help` command: `%s`.", arg);
                goto error;
            }
    ...
    if (!plugin_comp_cls_arg) {
        BT_CLI_LOGE_APPEND_CAUSE("Missing plugin name or component class descriptor.");
        goto error;
    }
    ...
    if (!substring) {
        BT_CLI_LOGE_APPEND_CAUSE("Could not consume argument: arg=%s", plugin_comp_cls_arg);
        goto error;
    }

    if (end_pos == strlen(plugin_comp_cls_arg)) {
        /* Didn't find an unescaped dot, treat it as a plugin name. */
        g_string_assign(cfg->cmd_data.help.cfg_component->plugin_name, plugin_comp_cls_arg);
    } else {
        /*
        ...
        if (!plugin_name || !comp_cls_name) {
            BT_CLI_LOGE_APPEND_CAUSE("Could not parse argument as a component class name: arg=%s",
                                     plugin_comp_cls_arg);
            goto error;
        }

        g_string_assign(cfg->cmd_data.help.cfg_component->plugin_name, plugin_name);
        g_string_assign(cfg->cmd_data.help.cfg_component->comp_cls_name, comp_cls_name);
    }
 ...
 * Prints the help command usage.
 */
static void print_query_usage(FILE *fp)
{
...
static const struct argpar_opt_descr query_options[] = {
    /* id, short_name, long_name, with_arg */
    {OPT_HELP, 'h', "help", false},
 ...
 * command.
 */
static enum bt_config_cli_args_status bt_config_query_from_args(int argc, const char *argv[],
                                                                struct bt_config **cfg_out,
                                                                const bt_value *plugin_paths,
                                                                int default_log_level,
                                                                unsigned int consumed_args)
{
    ...
    while (true) {
        enum parse_next_item_status parse_status =
            parse_next_item(argpar_iter, &argpar_item, argv, "query", consumed_args);
        ...
        if (argpar_item_type(argpar_item) == ARGPAR_ITEM_TYPE_OPT) {
            ...
            case OPT_PARAMS:
            {
                ...
                if (extend_status) {
                    BT_CLI_LOGE_APPEND_CAUSE(
                        "Cannot extend current parameters with --params option's argument:\n    %s",
                        arg);
                    goto error;
                }
                break;
            ...
            if (!component_class_spec) {
                ...
                BT_CLI_LOGE_APPEND_CAUSE(
                    "Extraneous command-line argument specified to `query` command: `%s`.", arg);
                goto error;
            }
        }
    }
    ...
    cfg->cmd_data.query.cfg_component =
        bt_config_component_from_arg(component_class_spec, default_log_level);
 ...
 */
static void print_list_plugins_usage(FILE *fp)
{
    ...
    fprintf(fp, "\n");
    fprintf(fp,
            "Use `babeltrace2 list-plugin-providers` to list the available plugin providers.\n");
}

static const struct argpar_opt_descr list_plugins_options[] = {
    /* id, short_name, long_name, with_arg */
    {OPT_HELP, 'h', "help", false},
    ARGPAR_OPT_DESCR_SENTINEL};
 ...
 */
static enum bt_config_cli_args_status bt_config_list_plugins_from_args(int argc, const char *argv[],
                                                                       struct bt_config **cfg_out,
                                                                       const bt_value *plugin_paths,
                                                                       unsigned int consumed_args)
{
    ...
    while (true) {
        enum parse_next_item_status parse_status =
            parse_next_item(argpar_iter, &argpar_item, argv, "list-plugins", consumed_args);
 ...
 */
static void print_list_plugin_providers_usage(FILE *fp)
{
...
static const struct argpar_opt_descr list_plugin_providers_options[] = {
    /* id, short_name, long_name, with_arg */
    {OPT_HELP, 'h', "help", false},
    ARGPAR_OPT_DESCR_SENTINEL};
 ...
 */
static enum bt_config_cli_args_status
bt_config_list_plugin_providers_from_args(int argc, const char *argv[], struct bt_config **cfg_out,
                                          unsigned int consumed_args)
{
    enum bt_config_cli_args_status status;
    struct bt_config *cfg = NULL;
    ...
    while (true) {
        enum parse_next_item_status parse_status = parse_next_item(
            argpar_iter, &argpar_item, argv, "list-plugin-providers", consumed_args);
 ...
 */
static void print_run_usage(FILE *fp)
{
    /* clang-format off */
 ...
 * command.
 */
static enum bt_config_cli_args_status bt_config_run_from_args(int argc, const char *argv[],
                                                              struct bt_config **cfg_out,
                                                              const bt_value *plugin_paths,
                                                              int default_log_level,
                                                              unsigned int consumed_args)
{
    ...
    while (true) {
        ...
        parse_status = parse_next_item(argpar_iter, &argpar_item, argv, "run", consumed_args);
        if (parse_status == PARSE_NEXT_ITEM_STATUS_ERROR) {
            goto error;
        } else if (parse_status == PARSE_NEXT_ITEM_STATUS_END) {
            break;
        ...
        case OPT_COMPONENT:
        {
            ...
            cur_cfg_comp = bt_config_component_from_arg(arg, default_log_level);
            if (!cur_cfg_comp) {
                BT_CLI_LOGE_APPEND_CAUSE(
            ...
            if (bt_value_copy(cur_base_params, &cur_cfg_comp->params) < 0) {
                BT_CLI_LOGE_APPEND_CAUSE_OOM();
                goto error;
            }

            ret = add_run_cfg_comp_check_name(cfg, cur_cfg_comp, dest, instance_names);
            if (ret) {
                goto error;
            }
        ...
        case OPT_PARAMS:
        {
            ...
            extend_status = bt_value_map_extend(cur_cfg_comp->params, params);
            BT_VALUE_PUT_REF_AND_RESET(params);
            if (extend_status != BT_VALUE_MAP_EXTEND_STATUS_OK) {
                BT_CLI_LOGE_APPEND_CAUSE(
                    "Cannot extend current component parameters with --params option's argument:\n    %s",
                    arg);
                goto error;
            }
        ...
        case OPT_LOG_LEVEL:
            if (!cur_cfg_comp) {
                BT_CLI_LOGE_APPEND_CAUSE(
                    "Cannot set the log level of unavailable component:\n    %s", arg);
                goto error;
            }
        ...
        case OPT_BASE_PARAMS:
        {
            ...
            if (!params) {
                BT_CLI_LOGE_APPEND_CAUSE(
                    "Invalid format for --base-params option's argument:\n    %s", error_str->str);
                goto error;
            }
        ...
        case OPT_RESET_BASE_PARAMS:
            ...
            break;
        case OPT_CONNECT:
            if (bt_value_array_append_string_element(connection_args, arg)) {
                BT_CLI_LOGE_APPEND_CAUSE_OOM();
                goto error;
            }
            break;
        case OPT_RETRY_DURATION:
        {
            ...
            if (retry_duration < 0) {
                BT_CLI_LOGE_APPEND_CAUSE(
                    "--retry-duration option's argument must be positive or 0: %ld",
                    retry_duration);
                goto error;
            }

            cfg->cmd_data.run.retry_duration_us = (uint64_t) retry_duration;
            break;
        }
        case OPT_ALLOWED_MIP_VERSIONS:
        {
            ...
            if (allowed_mip_version < 0 || allowed_mip_version > 1) {
                BT_CLI_LOGE_APPEND_CAUSE(
                    "--allowed-mip-versions option's argument must be 0 or 1: %ld",
                    allowed_mip_version);
                goto error;
            }
    ...
    ret = bt_config_cli_args_create_connections(cfg, connection_args, error_buf, 256);
    if (ret) {
...
static enum bt_config_cli_args_status bt_config_run_from_args_array(const bt_value *run_args,
                                                                    struct bt_config **cfg,
                                                                    const bt_value *plugin_paths,
                                                                    int default_log_level)
{
    ...
    for (i = 0; i < len; i++) {
        const bt_value *arg_value = bt_value_array_borrow_element_by_index_const(run_args, i);
        const char *arg;
    ...
    status = bt_config_run_from_args((int) len, argv, cfg, plugin_paths, default_log_level, 0);
 ...
 */
static void print_convert_usage(FILE *fp)
{
    /* clang-format off */
...
static const struct argpar_opt_descr convert_options[] = {
    /* id, short_name, long_name, with_arg */
    {OPT_ALLOWED_MIP_VERSIONS, 'm', "allowed-mip-versions", true},
...
static GString *get_component_auto_name(const char *prefix, const bt_value *existing_names)
{
    unsigned int i = 0;
...
static int assign_name_to_implicit_component(struct implicit_component_args *args,
                                             ...
                                             GList **comp_names, bool append_to_comp_names)
{
    ...
    if (bt_value_map_insert_entry(existing_names, name->str, bt_value_null)) {
        BT_CLI_LOGE_APPEND_CAUSE_OOM();
        ret = -1;
        goto end;
    }
...
static int append_run_args_for_implicit_component(struct implicit_component_args *impl_args,
                                                  bt_value *run_args)
{
    int ret = 0;
    uint64_t i;
    ...
    BT_ASSERT(!strchr(impl_args->name_arg->str, ':'));
    g_string_printf(component_arg_for_run, "%s:%s", impl_args->name_arg->str,
                    impl_args->comp_arg->str);
    ...
    if (bt_value_array_append_string_element(run_args, component_arg_for_run->str)) {
        BT_CLI_LOGE_APPEND_CAUSE_OOM();
        goto error;
    }
    ...
    if (impl_args->params_arg->len > 0) {
        ...
        if (bt_value_array_append_string_element(run_args, impl_args->params_arg->str)) {
            BT_CLI_LOGE_APPEND_CAUSE_OOM();
            goto error;
        }
    }
    ...
    for (i = 0; i < bt_value_array_get_length(impl_args->extra_params); i++) {
        ...
        elem = bt_value_array_borrow_element_by_index(impl_args->extra_params, i);
...
static void finalize_implicit_component_args(struct implicit_component_args *args)
{
    BT_ASSERT(args);
...
static void destroy_implicit_component_args(struct implicit_component_args *args)
{
...
static int init_implicit_component_args(struct implicit_component_args *args, const char *comp_arg,
                                        bool exists)
{
    int ret = 0;
    ...
    if (!args->comp_arg || !args->name_arg || !args->params_arg || !args->extra_params) {
        ret = -1;
        finalize_implicit_component_args(args);
        BT_CLI_LOGE_APPEND_CAUSE_OOM();
        goto end;
...
static struct implicit_component_args *create_implicit_component_args(const char *comp_arg)
{
...
static void append_implicit_component_param(struct implicit_component_args *args, const char *key,
                                            const char *value)
{
    BT_ASSERT(args);
    BT_ASSERT(key);
...
static int append_multiple_implicit_components_param(GPtrArray *implicit_comp_args,
                                                     const char *comp_arg, const char *key,
                                                     const char *value)
{
    int i;
    int n = 0;
...
static gchar *escape_string_value(const char *value)
{
    GString *ret;
    const char *in;
...
static int bt_value_to_cli_param_value_append(const bt_value *value, GString *buf)
{
    BT_ASSERT(buf);
    ...
    case BT_VALUE_TYPE_ARRAY:
    {
        ...
        for (uint64_t i = 0; i < sz; i++) {
            ...
            item = bt_value_array_borrow_element_by_index_const(value, i);
...
static gchar *bt_value_to_cli_param_value(bt_value *value)
{
    GString *buf;
    gchar *result = NULL;
...
static int append_parameter_to_args(bt_value *args, const char *key, bt_value *value)
{
    BT_ASSERT(args);
...
static int append_string_parameter_to_args(bt_value *args, const char *key, const char *value)
{
    bt_value *str_value;
    int ret;
...
static int append_implicit_component_extra_param(struct implicit_component_args *args,
 ...
 */
static GString *escape_dot_colon(const char *input)
{
 ...
 * function.
 */
static int append_connect_arg(bt_value *run_args, const char *upstream_name,
 ...
 */
static int convert_auto_connect(bt_value *run_args, GList *source_names, GList *filter_names,
                                GList *sink_names)
{
    int ret = 0;
    ...
    for (source_at = source_names; source_at; source_at = g_list_next(source_at)) {
        ...
        ret = append_connect_arg(run_args, source_name->str, filter_name->str);
        if (ret) {
            goto error;
        }
    }
    ...
    for (; filter_at; filter_prev = filter_at, filter_at = g_list_next(filter_at)) {
        ...
        ret = append_connect_arg(run_args, filter_prev_name->str, filter_name->str);
        if (ret) {
            goto error;
        }
    }
    ...
    for (sink_at = sink_names; sink_at; sink_at = g_list_next(sink_at)) {
        ...
        ret = append_connect_arg(run_args, filter_name->str, sink_name->str);
        if (ret) {
            goto error;
        }
    }
...
static int g_list_prepend_gstring(GList **list, const char *string)
{
    int ret = 0;
...
static int create_implicit_component_args_from_auto_discovered_sources(
    const struct auto_source_discovery *auto_disc, const bt_value *non_opts,
    const bt_value *non_opt_params, const bt_value *non_opt_loglevels, GPtrArray *component_args)
{
    gchar *cc_name = NULL;
    ...
    for (i = 0; i < len; i++) {
        ...
        for (orig_indices_i = 0; orig_indices_i < orig_indices_count; orig_indices_i++) {
            const bt_value *orig_idx_value =
                bt_value_array_borrow_element_by_index(res->original_input_indices, orig_indices_i);
            uint64_t orig_idx = bt_value_integer_unsigned_get(orig_idx_value);
            const bt_value *params_array =
                bt_value_array_borrow_element_by_index_const(non_opt_params, orig_idx);
            ...
            for (params_i = 0; params_i < params_count; params_i++) {
                const bt_value *params_value =
                    bt_value_array_borrow_element_by_index_const(params_array, params_i);
                ...
                append_status = bt_value_array_append_string_element(comp->extra_params, params);
                if (append_status != BT_VALUE_ARRAY_APPEND_ELEMENT_STATUS_OK) {
                    BT_CLI_LOGE_APPEND_CAUSE("Failed to append array element.");
                    goto error;
                }
         ...
         * found trace.
         */
        if (bt_value_array_get_length(res->inputs) == 1 && strcmp(res->plugin_name, "ctf") == 0 &&
            strcmp(res->source_cc_name, "fs") == 0) {
            const bt_value *orig_idx_value =
                bt_value_array_borrow_element_by_index(res->original_input_indices, 0);
            uint64_t orig_idx = bt_value_integer_unsigned_get(orig_idx_value);
            const bt_value *non_opt_value =
                bt_value_array_borrow_element_by_index_const(non_opts, orig_idx);
            const char *non_opt = bt_value_string_get(non_opt_value);
            const bt_value *input_value =
                bt_value_array_borrow_element_by_index_const(res->inputs, 0);
            ...
            if (strlen(input) > 0) {
                append_string_parameter_to_args(comp->extra_params, "trace-name", input);
            }
        }
 ...
 */
static enum bt_config_cli_args_status
bt_config_convert_from_args(int argc, const char *argv[], struct bt_config **cfg_out,
                            const bt_value *plugin_paths, int *default_log_level,
                            const bt_interrupter *interrupter, unsigned int consumed_args)
{
    enum bt_config_cli_args_status status;
    enum convert_current_item_type current_item_type = CONVERT_CURRENT_ITEM_TYPE_NONE;
    int ret;
    struct bt_config *cfg = NULL;
    ...
    if (init_implicit_component_args(&implicit_ctf_output_args, "sink.ctf.fs", false)) {
        goto error;
    }

    if (init_implicit_component_args(&implicit_lttng_live_args, "source.ctf.lttng-live", false)) {
        goto error;
    }

    if (init_implicit_component_args(&implicit_text_args, "sink.text.pretty", false)) {
        goto error;
    }
    ...
    if (init_implicit_component_args(&implicit_debug_info_args, "filter.lttng-utils.debug-info",
                                     false)) {
        goto error;
    }

    if (init_implicit_component_args(&implicit_muxer_args, "filter.utils.muxer", true)) {
        goto error;
    }

    if (init_implicit_component_args(&implicit_trimmer_args, "filter.utils.trimmer", false)) {
        goto error;
    }
    ...
    while (true) {
        ...
        parse_status = parse_next_item(argpar_iter, &argpar_item, argv, "convert", consumed_args);
        if (parse_status == PARSE_NEXT_ITEM_STATUS_ERROR) {
            goto error;
        } else if (parse_status == PARSE_NEXT_ITEM_STATUS_END) {
            break;
        ...
        if (argpar_item_type(argpar_item) == ARGPAR_ITEM_TYPE_OPT) {
            ...
            case OPT_COMPONENT:
            {
                ...
                if (name) {
                    ...
                    g_string_printf(component_arg_for_run, "%s:%s", name_gstr->str, arg);
                }
                 ...
                 * all component names.
                 */
                if (bt_value_map_insert_entry(all_names, name_gstr->str, bt_value_null)) {
                    BT_CLI_LOGE_APPEND_CAUSE_OOM();
                    goto error;
                }
            ...
            case OPT_LOG_LEVEL:
                ...
                break;
            case OPT_RETRY_DURATION:
                if (bt_value_array_append_string_element(run_args, "--retry-duration")) {
                    BT_CLI_LOGE_APPEND_CAUSE_OOM();
                    goto error;
                }
                ...
                break;
            case OPT_ALLOWED_MIP_VERSIONS:
                if (bt_value_array_append_string_element(run_args, "--allowed-mip-versions")) {
                    BT_CLI_LOGE_APPEND_CAUSE_OOM();
                    goto error;
                }
            ...
            append_status = bt_value_array_append_empty_array_element(non_opt_params, NULL);
            if (append_status != BT_VALUE_ARRAY_APPEND_ELEMENT_STATUS_OK) {
                BT_CLI_LOGE_APPEND_CAUSE_OOM();
                goto error;
            }
    ...
    while (true) {
        ...
        case OPT_BEGIN:
            ...
            ret = append_implicit_component_extra_param(&implicit_trimmer_args, "begin", arg);
        ...
        case OPT_END:
            ...
            ret = append_implicit_component_extra_param(&implicit_trimmer_args, "end", arg);
            implicit_trimmer_args.exists = true;
            if (ret) {
                goto error;
            }
        ...
        case OPT_TIMERANGE:
        {
            ...
            ret = append_implicit_component_extra_param(&implicit_trimmer_args, "begin", begin);
            ret |= append_implicit_component_extra_param(&implicit_trimmer_args, "end", end);
            implicit_trimmer_args.exists = true;
            free(begin);
            free(end);
            if (ret) {
        ...
        case OPT_CLOCK_CYCLES:
            append_implicit_component_param(&implicit_text_args, "clock-cycles", "yes");
            implicit_text_args.exists = true;
            break;
        case OPT_CLOCK_DATE:
            append_implicit_component_param(&implicit_text_args, "clock-date", "yes");
            implicit_text_args.exists = true;
            break;
        case OPT_CLOCK_FORCE_CORRELATE:
            ...
            break;
        case OPT_CLOCK_GMT:
            append_implicit_component_param(&implicit_text_args, "clock-gmt", "yes");
            append_implicit_component_param(&implicit_trimmer_args, "gmt", "yes");
            implicit_text_args.exists = true;
            break;
        case OPT_CLOCK_OFFSET:
        ...
        case OPT_CLOCK_OFFSET_NS:
            ...
            break;
        case OPT_CLOCK_SECONDS:
            append_implicit_component_param(&implicit_text_args, "clock-seconds", "yes");
            implicit_text_args.exists = true;
            break;
        case OPT_COLOR:
            implicit_text_args.exists = true;
            ret = append_implicit_component_extra_param(&implicit_text_args, "color", arg);
            if (ret) {
                goto error;
            }
            break;
        ...
        case OPT_DEBUG_INFO_DIR:
            implicit_debug_info_args.exists = true;
            ret = append_implicit_component_extra_param(&implicit_debug_info_args, "debug-info-dir",
                                                        arg);
            if (ret) {
                goto error;
            }
        ...
        case OPT_DEBUG_INFO_FULL_PATH:
            implicit_debug_info_args.exists = true;
            append_implicit_component_param(&implicit_debug_info_args, "full-path", "yes");
            break;
        case OPT_DEBUG_INFO_TARGET_PREFIX:
            implicit_debug_info_args.exists = true;
            ret = append_implicit_component_extra_param(&implicit_debug_info_args, "target-prefix",
                                                        arg);
            if (ret) {
                goto error;
            }
        ...
        case OPT_FIELDS:
        {
            ...
            implicit_text_args.exists = true;
            ret = insert_flat_params_from_array(implicit_text_args.params_arg, fields, "field");
        ...
        case OPT_NAMES:
        {
            ...
            implicit_text_args.exists = true;
            ret = insert_flat_params_from_array(implicit_text_args.params_arg, names, "name");
        ...
        case OPT_NO_DELTA:
            append_implicit_component_param(&implicit_text_args, "no-delta", "yes");
            implicit_text_args.exists = true;
            break;
        case OPT_INPUT_FORMAT:
        ...
        case OPT_STREAM_INTERSECTION:
            ...
            break;
        case OPT_VERBOSE:
            *default_log_level = logging_level_min(*default_log_level, BT_LOG_INFO);
            break;
        case OPT_DEBUG:
            *default_log_level = logging_level_min(*default_log_level, BT_LOG_TRACE);
            break;
        case OPT_ALLOWED_MIP_VERSIONS:
        case OPT_COMPONENT:
        case OPT_HELP:
    ...
    if (*default_log_level == BT_LOG_INFO) {
        append_implicit_component_param(&implicit_text_args, "verbose", "yes");
    }
    ...
    if (print_ctf_metadata) {
        ...
        g_string_assign(cfg->cmd_data.print_ctf_metadata.path, bt_value_string_get(bt_val_non_opt));

        if (output) {
            g_string_assign(cfg->cmd_data.print_ctf_metadata.output_path, output);
        }
    ...
    if (implicit_ctf_output_args.exists) {
        ...
        if (implicit_text_args.exists) {
            BT_CLI_LOGE_APPEND_CAUSE(
                "Ambiguous --output option: --output-format=ctf specified but another option implies --output-format=text.");
            goto error;
        }
    }
     ...
     */
    if (!implicit_dummy_args.exists && !implicit_ctf_output_args.exists && !sink_names) {
        implicit_text_args.exists = true;
    }
    ...
    if (output) {
        if (implicit_text_args.exists) {
            append_implicit_component_extra_param(&implicit_text_args, "path", output);
        } else if (implicit_ctf_output_args.exists) {
            append_implicit_component_extra_param(&implicit_ctf_output_args, "path", output);
        }
    }
    ...
    if (bt_value_array_get_length(non_opts) > 0) {
        if (implicit_lttng_live_args.exists) {
            ...
            bt_val_non_opt = bt_value_array_borrow_element_by_index_const(non_opts, 0);
            lttng_live_url_parts = bt_common_parse_lttng_live_url(
                bt_value_string_get(bt_val_non_opt), error_buf, sizeof(error_buf));
            if (!lttng_live_url_parts.proto) {
                BT_CLI_LOGE_APPEND_CAUSE("Invalid LTTng live URL format: %s.", error_buf);
                goto error;
            }

            if (!lttng_live_url_parts.session_name) {
                /* Print LTTng live sessions */
                status = bt_config_print_lttng_live_sessions_create(plugin_paths, &cfg);
                if (status != BT_CONFIG_CLI_ARGS_STATUS_OK) {
                    goto end;
                }
                ...
                if (output) {
                    g_string_assign(cfg->cmd_data.print_lttng_live_sessions.output_path, output);
                }
            ...
            if (bt_value_array_append_string_element(live_inputs_array_val,
                                                     bt_value_string_get(bt_val_non_opt))) {
                BT_CLI_LOGE_APPEND_CAUSE_OOM();
                goto error;
            }

            ret = append_parameter_to_args(implicit_lttng_live_args.extra_params, "inputs",
                                           live_inputs_array_val);
            if (ret) {
                goto error;
            }

            ret = append_implicit_component_extra_param(&implicit_lttng_live_args,
            ...
            ret = create_implicit_component_args_from_auto_discovered_sources(
                &auto_disc, non_opts, non_opt_params, non_opt_loglevels, discovered_source_args);
            if (ret != 0) {
                goto error;
            }
        }
    ...
    if (ctf_fs_source_force_clock_class_unix_epoch_origin) {
        ...
        n = append_multiple_implicit_components_param(discovered_source_args, "source.ctf.fs",
                                                      "force-clock-class-origin-unix-epoch", "yes");
        if (n == 0) {
            BT_CLI_LOGE_APPEND_CAUSE(
                "--clock-force-correlate specified, but no source.ctf.fs component instantiated.");
            goto error;
        }
    }
    ...
    if (ctf_fs_source_clock_class_offset_ns_arg) {
        ...
        if (n == 0) {
            BT_CLI_LOGE_APPEND_CAUSE(
                "--clock-offset-ns specified, but no source.ctf.fs component instantiated.");
            goto error;
        }
    }
    ...
    for (i = 0; i < discovered_source_args->len; i++) {
        ...
        args = static_cast<implicit_component_args *>(discovered_source_args->pdata[i]);
        ...
        ret = assign_name_to_implicit_component(args, auto_disc_comp_name->str, all_names,
                                                &source_names, true);
        if (ret) {
            goto error;
        }
    ...
    ret = assign_name_to_implicit_component(&implicit_lttng_live_args, "lttng-live", all_names,
                                            &source_names, true);
    if (ret) {
        goto error;
    }

    ret = assign_name_to_implicit_component(&implicit_text_args, "pretty", all_names, &sink_names,
                                            true);
    if (ret) {
        goto error;
    }

    ret = assign_name_to_implicit_component(&implicit_ctf_output_args, "sink-ctf-fs", all_names,
                                            &sink_names, true);
    if (ret) {
        goto error;
    }

    ret = assign_name_to_implicit_component(&implicit_dummy_args, "dummy", all_names, &sink_names,
                                            true);
    if (ret) {
        goto error;
    }

    ret = assign_name_to_implicit_component(&implicit_muxer_args, "muxer", all_names, NULL, false);
    if (ret) {
        goto error;
    }

    ret = assign_name_to_implicit_component(&implicit_trimmer_args, "trimmer", all_names, NULL,
                                            false);
    if (ret) {
        goto error;
    }

    ret = assign_name_to_implicit_component(&implicit_debug_info_args, "debug-info", all_names,
                                            NULL, false);
    if (ret) {
        goto error;
    }
    ...
    if (g_list_length(sink_names) != 1) {
        BT_CLI_LOGE_APPEND_CAUSE("More than one sink component specified.");
        goto error;
    }
    ...
    if (implicit_debug_info_args.exists) {
        if (g_list_prepend_gstring(&filter_names, implicit_debug_info_args.name_arg->str)) {
            goto error;
        }
    }

    if (implicit_trimmer_args.exists) {
        if (g_list_prepend_gstring(&filter_names, implicit_trimmer_args.name_arg->str)) {
            goto error;
        }
    }

    if (g_list_prepend_gstring(&filter_names, implicit_muxer_args.name_arg->str)) {
        goto error;
    }
    ...
    for (i = 0; i < discovered_source_args->len; i++) {
        struct implicit_component_args *args =
            static_cast<implicit_component_args *>(discovered_source_args->pdata[i]);
    ...
    ret = append_run_args_for_implicit_component(&implicit_lttng_live_args, run_args);
    if (ret) {
        goto error;
    }

    ret = append_run_args_for_implicit_component(&implicit_text_args, run_args);
    if (ret) {
        goto error;
    }

    ret = append_run_args_for_implicit_component(&implicit_ctf_output_args, run_args);
    if (ret) {
        goto error;
    }

    ret = append_run_args_for_implicit_component(&implicit_dummy_args, run_args);
    if (ret) {
        goto error;
    }

    ret = append_run_args_for_implicit_component(&implicit_muxer_args, run_args);
    if (ret) {
        goto error;
    }

    ret = append_run_args_for_implicit_component(&implicit_trimmer_args, run_args);
    if (ret) {
        goto error;
    }

    ret = append_run_args_for_implicit_component(&implicit_debug_info_args, run_args);
    if (ret) {
        goto error;
    }

    /* Auto-connect components */
    ret = convert_auto_connect(run_args, source_names, filter_names, sink_names);
    if (ret) {
    ...
    if (print_run_args || print_run_args_0) {
        ...
        for (args_idx = 0; args_idx < args_len; args_idx++) {
            const bt_value *arg_value = bt_value_array_borrow_element_by_index(run_args, args_idx);
            const char *arg;
            GString *quoted = NULL;
    ...
    status = bt_config_run_from_args_array(run_args, &cfg, plugin_paths, *default_log_level);
    if (status != BT_CONFIG_CLI_ARGS_STATUS_OK) {
        goto end;
    }
 ...
 */
static void print_gen_usage(FILE *fp)
{
    /* clang-format off */
                          ...
                          const bt_value *initial_plugin_paths, const bt_interrupter *interrupter)
{
    ...
    if (!initial_plugin_paths) {
        ...
        bt_value_copy_status copy_status = bt_value_copy(initial_plugin_paths, &plugin_paths);
        if (copy_status) {
            goto error;
        }
    }
    ...
    while (true) {
        ...
        case ARGPAR_ITER_NEXT_STATUS_ERROR:
        {
            if (argpar_error_type(argpar_error) != ARGPAR_ERROR_TYPE_UNKNOWN_OPT) {
                GString *err_str = format_arg_error(argpar_error, top_level_argv, 0,
        ...
        if (argpar_status == ARGPAR_ITER_NEXT_STATUS_ERROR) {
            BT_ASSERT(argpar_error_type(argpar_error) == ARGPAR_ERROR_TYPE_UNKNOWN_OPT);
            /*
        ...
        if (argpar_item_type(argpar_item) == ARGPAR_ITEM_TYPE_OPT) {
            ...
            case OPT_LOG_LEVEL:
            {
                ...
                default_log_level = logging_level_min(default_log_level, level);
                break;
            }
            case OPT_PLUGIN_PATH:
                if (bt_config_append_plugin_paths_check_setuid_setgid(plugin_paths, arg)) {
                    goto error;
                }
                break;
            case OPT_OMIT_SYSTEM_PLUGIN_PATH:
     ...
     */
    if (append_home_and_system_plugin_paths(plugin_paths, omit_system_plugin_path,
    ...
    case COMMAND_TYPE_RUN:
        status = bt_config_run_from_args(command_argc, command_argv, cfg, plugin_paths,
                                         default_log_level, consumed_args);
        break;
    case COMMAND_TYPE_CONVERT:
        status = bt_config_convert_from_args(command_argc, command_argv, cfg, plugin_paths,
                                             &default_log_level, interrupter, consumed_args);
        break;
    case COMMAND_TYPE_LIST_PLUGINS:
        status = bt_config_list_plugins_from_args(command_argc, command_argv, cfg, plugin_paths,
                                                  consumed_args);
        break;
    case COMMAND_TYPE_LIST_PLUGIN_PROVIDERS:
        status = bt_config_list_plugin_providers_from_args(command_argc, command_argv, cfg,
                                                           consumed_args);
        break;
    case COMMAND_TYPE_HELP:
        status = bt_config_help_from_args(command_argc, command_argv, cfg, plugin_paths,
                                          default_log_level, consumed_args);
        break;
    case COMMAND_TYPE_QUERY:
        status = bt_config_query_from_args(command_argc, command_argv, cfg, plugin_paths,
                                           default_log_level, consumed_args);
        break;
    default:
        bt_common_abort();
