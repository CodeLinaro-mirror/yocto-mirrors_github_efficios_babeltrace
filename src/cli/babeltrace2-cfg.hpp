...
static inline struct bt_config_component *bt_config_get_component(GPtrArray *array, size_t index)
{
    struct bt_config_component *comp =
        static_cast<bt_config_component *>(g_ptr_array_index(array, index));

    bt_object_get_ref(comp);
    return comp;
}

int bt_config_append_plugin_paths(bt_value *plugin_paths, const char *arg);
