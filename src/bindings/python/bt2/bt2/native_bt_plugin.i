...
/* Output argument typemap for plugin provider set output (always appends) */
%typemap(in, numinputs=0)
	(const bt_plugin_provider_set **)
	(bt_plugin_provider_set *temp_plugin_provider_set = NULL) {
	$1 = &temp_plugin_provider_set;
}
...
%typemap(argout)
	...
	if (*$1) {
		...
		$result = SWIG_AppendOutput($result,
				SWIG_NewPointerObj(SWIG_as_voidptr(*$1),
					SWIGTYPE_p_bt_plugin_provider_set, 0));
	} else {
		/* SWIG_AppendOutput() steals Py_None */
		Py_INCREF(Py_None);
		$result = SWIG_AppendOutput($result, Py_None);
...
%include <babeltrace2/plugin/plugin-provider.h>
...
bt_plugin_provider_set_borrow_status bt_bt2_plugin_provider_set_borrow(
		const bt_plugin_provider_set **plugin_provider_set);

bt_property_availability bt_bt2_plugin_provider_get_version(
